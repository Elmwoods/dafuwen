...
class Redis
{
    ...
    private static $expire = 18000; //默认存储时间（秒）
    private static $host = '127.0.0.1';
    private static $port = '6379';
    private static $password = '';
    private static $db = 0;
    private static $timeout = 10;
    private static $pre = 'luoqizhun_';
    /**
     * 初始化Redis连接
    ...
    public function __construct($config=[])
    {
        ...
        $rs = self::$redis->connect(self::$host,self::$port,self::$timeout);
        if(!$rs)
    ...
    {
        $list1 = self::$pre . $list1;
        $list2 = self::$pre . $list2;
    ...
    {
        $list = self::$pre . $list;
        return self::$redis->lrem($list, $element, $count);
    }
     ...
     */
    public static function sadd($set, $value,$expire=0)
    {
        $num = 0;
        $set = self::$pre . $set;
        if(is_array($value)){
