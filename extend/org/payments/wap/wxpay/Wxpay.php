...
class Wxpay extends Wxpay_client_pub{
	...
	public function get_payform(&$order_info=[],$openid){
		...
		//是否有合并支付订单
		$out_trade_no = !empty($order_info['out_comm_order_sn']) ? $order_info['out_comm_order_sn']: $order_info['order_sn'];
		$unifiedOrder->setParameter("openid",$openid);
		$unifiedOrder->setParameter("body",$out_trade_no);//商品描述remark
		//自定义订单号，此处仅作举例
		...
		$unifiedOrder->setParameter("trade_type","APP");//交易类型 JSAPI，NATIVE，APP
		$unifiedOrder->setParameter("attach","wxapp");//	附加数据，在查询API和支付通知中原样返回，可作为自定义参数使用 String(127)
		...
		if (!empty($prepay_id)) {	//返回客户端的参数  
			...
			//$info['out_comm_order_sn'] = $order_info['out_comm_order_sn'];
			$info['orderId'] = $order_info['out_comm_order_sn'];
	 ...
	 *  $param array $order_info 订单信息
	 */
	public function get_payform2(&$order_info=[],$openid){
		if(!$order_info){
			return false;
		}
		...
		$unifiedOrder->setParameter("attach","wxpub_app");//	附加数据，在查询API和支付通知中原样返回，可作为自定义参数使用 
