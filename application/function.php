...
function get_arr_column($arr, $key_name)
{
	$arr2 = array();
	foreach($arr as $key => $val){
		$arr2[] = isset($val[$key_name])?$val[$key_name] :'';
	}
	return $arr2;
}
...
function check_mobile($mobile){
    if(preg_match('/1[13456789]\d{9}$/',$mobile))
        return true;
    return false;
}
...
function get_end_time(){
    $ge_time['min'] = config('end_time');
    $ge_time['fen'] = $ge_time['min']/60;
    $ge_time['fen'] = round($ge_time['fen'],1);
    return $ge_time;
}
/**
 * 过滤包含脚本的图片
