...
class User extends Base{
    ...
    public function help(){
      ...
      DataReturn::returnBase64Json(200,'校验成功',html_entity_decode($article["content"]));
    ...
    public function index()
    {
        ...
        $users['contract_revenue']   = db('account_log')->where(['user_id'=>$user_id,'type'=>21])->sum('contract_revenue');//合约收益
        ...
        $users['user_identity'] = 1;
        $identity_id = Db::name('user_identity')->where('user_id',$user_id)->order("id desc")->find();
        if($identity_id){
             $users['user_identity'] = Db::name('user_identity')->where('user_id',$user_id)->value('status');
        }
        // var_dump($identity_id['status']);die;
    ...
    public function user_mobile(){
        ...
        $users_is = M('users')->where('user_id', $user_id)->find();
        
        DataReturn::returnBase64Json(200,'数据返回成功',$users_is);
    }
    ...
    public  function authentication(){
        if (IS_POST) {
            ...
            //自动审核
            // $host = "http://vipidcardcheck.haoservice.com";
            // $path = "/idcard/VerifyIdcardv2";
            // $method = "GET";
            // $appcode = "21832ea906b64cbbbe6d5d2d202264c6";
            // $headers = array();
            // array_push($headers, "Authorization:APPCODE " . $appcode);
            // $querys = "cardNo=".$data['identity']."&realName=".$data['real_name'];
            // $bodys = "";
            // $url = $host . $path . "?" . $querys;
            // $res=json_decode($this->curl_get($url,$headers),true);
            // if($res['reason']){
            //     if($res['result']['isok']){
            //         $data['status']=1;
            //         db('users')->where('user_id',$this->user_id)->update(['identity'=>$data['identity'],'real_name'=>$data['real_name']]);
            //     }else{
            //         DataReturn::returnJson(500,'您输入的姓名和身份号信息不符！',[]);
            //     }
            // }else{
            //     DataReturn::returnJson(500,'接口错误，请联系客服',[]);
            // }
           
            $res = db('user_identity')->add($data);
            if ($res) {
                DataReturn::returnJson(200,'实名验证成功',[]);
            }else{
 ...
 /*       $url = SITE_URL.'/dist/pages/register.html?first_leader='.$this->user_info['mobile'];
        ...
        $image->water('./'.$after_path , Image::WATER_CENTER, 100)
            ->text($this->user_info['mobile'],ROOT_PATH . 'public/static/assets/ttfs/microsoft.ttf',23,'#ffffff', Image::WATER_CENTER,[0,120])
            ->text($nick_name,ROOT_PATH . 'public/static/assets/ttfs/microsoft.ttf',23,'#ffffff', Image::WATER_CENTER,[0,180]);
        $image->save($path);
    ...
    public function password()
    {
        if (request()->isPost()) {
            ...
            $code_status = Code::check($code_s,5);
            if($code_status['status'] != 1){
                DataReturn::returnJson(0, $code_status['message'], []);
            }


            $sms_log  =  db('sms_log')->where(['mobile' => $mobile, 'status' => 1])->order('id desc')->field('id,code, add_time')->find();
    ...
    public function transfer()
    {
        if(request()->isPost()){
            ...
            Db::startTrans();
            // try{
                $res1 = Db::name('users')->where('user_id',$post['user_id'])->setInc('pay_points',$post['nums']);//被转让人增加微分
                $res2 = Db::name('users')->where('user_id',$post['seller_id'])->setDec('pay_points',$post['nums']);//转让者减少微分
                $res3 = accountLog1($post['seller_id'],0,0,'-'.$post['nums'],'用户:'.$post['seller_id'].'转让'.$post['nums'].'微分到用户:'.$post['user_id'],2);
                //资金流日志
                $res4 = accountLog1($post['user_id'],0,0,$post['nums'],'用户收入'.$post['seller_id'].'转让'.$post['nums'].'微分',2);//资金流日志
                // 提交事务
