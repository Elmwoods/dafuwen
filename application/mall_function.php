...
function queryExpress($postcom , $getNu) {
    ...
    $url = "https://www.kuaidi100.com/query?type=".$postcom."&postid=".$getNu."&valicode=&temp=0.49738534969422676";
...
function getCatGrandson ($cat_id)
{
    ...
    // 把整张表找出来
    $GLOBALS['category_id_arr'] = M('GoodsCategory')->cache(true,ZPSHOP_CACHE_TIME)->getField('id,parent_id');
    // 先把所有儿子找出来
    $son_id_arr = M('GoodsCategory')->where("parent_id", $cat_id)->cache(true,ZPSHOP_CACHE_TIME)->getField('id',true);
...
function tpCache($config_key,$data = array()){
    ...
    if(empty($data)){
		...
		$group_id = $type_arr[$param[0]];
        if(empty($config)){
            //缓存文件不存在就读取数据库
            $res = D('config')->where("group",$group_id)->select();
            if($res){
        ...
        //更新缓存
        $result =  D('config')->where("group", $group_id)->select();
        if($result){
            foreach($result as $val){
                $temp[$val['name']] = $val['value'];
            }
            foreach ($data as $k=>$v){
                $newArr = array('name'=>$k,'value'=>trim($v),'group'=>$group_id);
                if(!isset($temp[$k])){
            ...
            //更新后的数据库记录
            $newRes = D('config')->where("group", $group_id)->select();
 ...
 * @param string $type 1:购买2:转赠
 * @return  bool
 */
function accountLog($user_id, $user_money = 0,$pay_points = 0, $desc = '',$distribut_money = 0,$order_id = 0 ,$order_sn = '',$type,$pig_id = 0){
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'       => $user_id,
...
//        " pay_points = pay_points + $pay_points, distribut_money = distribut_money + $distribut_money WHERE user_id = $user_id";
   ...
   // 为推广收益做加盐处理-------------
    $userdate = Db::name('users')->where(array('user_id'=>$user_id))->field('user_money,usermoneysalt')->find();
    $usermoneysalt=$userdate["usermoneysalt"];
    
    if($user_money!=0){  //如果推广收益不为零，就加盐        
       $salt_money=$userdate["user_money"]+$user_money;
...
//    if($user_money > 0)
//        $update_data['accumulated_income'] += $user_money;
...
/**
 * 记录帐户变动 不增加user_money , pay_points
 * @param   int     $user_id        用户id
 * @param   float   $user_money     可用余额变动
 * @param   float   $frozen_money     冻结余额变动
 * @param   int     $pay_points     消费积分变动
 ...
 * @return  bool
 */
function accountLog1($user_id, $user_money = 0,$frozen_money = 0,$pay_points = 0, $desc = '',$type){
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'       => $user_id,
        'user_money'    => $user_money,
    ...
    if(M('account_log')->add($account_log)){
        return true;
    }else{
        return false;
    }
 ...
 * 记录帐户变动 
 * @param   int     $user_id        用户id
 * @param   float   $pig_currency  $doge_money        金额变动
 * @param   string  $desc    变动说明
 * @param   string  $type 1:pig币2:doge币
 * @return  bool
 */
function accountLog2($user_id, $pig_currency = 0,$doge_money = 0,$desc = '',$type){
    /* 插入帐户变动记录 */
    $log = array(
        'user_id'       => $user_id,
    ...
    //更新用户信息
    $update_data = array(
        'pig_currency'        => ['exp','pig_currency+'.$pig_currency],
        'doge_money'          => ['exp','doge_money+'.$doge_money],
    );
    if(($pig_currency+$doge_money) == 0)
        return false;
    $update = Db::name('users')->where('user_id',$user_id)->update($update_data);
    if($update){
        M('pig_doge_money')->add($log);
        return true;
    }else{
        return false;
    }
...
function orderBtn($order_id = 0, $order = array())
{
    ...
    if($order['pay_status'] == 1  && $order['shipping_status']==1 && $order['order_status'] == 4) // 已完成(已支付, 已发货 , 已完成)
    {
...
//             }
        ...
        //累计消费写入
        $total_amount = M('order')->master()->where("user_id=:user_id AND pay_status=1 and order_status not in (3,5)")->bind(['user_id'=>$order['user_id']])->sum('integral_money');
        $updata_amount['total_amount'] = $total_amount;//更新累计修复额度
        M('users')->where("user_id", $order['user_id'])->save($updata_amount);
        ...
        if ($order['order_prom_type'] == 6) {
         ...
         //发票生成
        $Invoice = new \app\shop\logic\InvoiceLogic();
...
function calculate_price($user_id = 0, $order_goods, $shipping_code = '', $shipping_price = 0, $province = 0, $city = 0, $district = 0, $pay_points = 0, $user_money = 0, $coupon_id = 0)
{
  ...
  /*  if(($use_percent_point !=1 ) && $pay_points > $result['order_integral']) {
    ...
    $goods_arr = M('goods')->where("goods_id in(" . implode(',', $goods_id_arr) . ")")->cache(true,ZPSHOP_CACHE_TIME)
    ...
    $integral_money = $pay_points>0 ? ($pay_points / $point_rate) : 0;
...
function update_stock_log($muid, $stock = 1, $goods, $order_sn = '')
{
    ...
    $data['goods_name'] = empty($goods['goods_name']) ? '' : $goods['goods_name'];
...
function updateRefundOrder($order,$type=0){
    ...
    M('order')->where(array('order_id'=>$order['order_id']))->save(array('pay_status'=>3,'admin_note'=>$order['admin_note'])); //更改订单状态
    $orderLogic = new app\common\logic\OrderLogic();
